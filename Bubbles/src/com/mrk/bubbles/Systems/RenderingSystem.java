	...
	public void processOneGameTick(long lastFrameTime) {
		for (UUID entity : entityManager.getAllEntitiesPossessingComponent(TextureComponent.class)) {
			...
			if (meta.has(PositionComponent.class) && meta.has(DimensionComponent.class))
				GraphicsHelper.drawTexture(MRKOGL.getInstance().gl, MRKOGL.getInstance().graphics.getTexture(meta.get(TextureComponent.class).texture), meta.get(PositionComponent.class).x, meta.get(PositionComponent.class).y, 1f, 0f, 0f, 0f);
		}
	}
